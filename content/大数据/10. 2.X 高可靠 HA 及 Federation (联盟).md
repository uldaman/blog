Title: 10. 2.X 高可靠 HA 及 Federation (联盟)
Author: Martin
...
## 配置 Federation
Federation 的配置管理是向后兼容的, 即允许在不改变任何配置的情况下将当前运行的单节点环境转换成联邦环境.

新的配置文件被设计成使得在集群中的所有节点都可以具有相同的配置文件，而不需要部署不同的配置文件在不同的机器上.

### 设置配置文件
配置 Federation 我好像找到两种方式 . . .具体还没弄明白, 一种是配置 **nameservices**, 另一种是配置 **viewfs**.

#### 方式一: nameservices
要使用 HDFS Federation, 只需在 DataNode 的 `hdfs-site.xml` 中修改如下选项即可:
...
<property>
    ...
    <description>
    ClusterName 是集群对外提供服务的 NS 逻辑名称, 可以自定义一个;
    注意, 这里的协议不再是 hdfs, 而是新引入的 viewfs, 这个逻辑名称会在下面的挂载表中用到
    </description>
</property>

<property>
    <name>fs.viewfs.mounttable.default.link./cluster01</name>
    <value>hdfs://itcast01:9000/</value>
</property>

<property>
    <name>fs.viewfs.mounttable.default.link./cluster02</name>
    <value>hdfs://itcast02:9000</value>
</property>
```
<br>
访问地址: `viewfs://ClusterName/cluster01` (以前的访问方式 `hdfs://itcast01:9000` 依然有效果).

### 格式化 NameNode
不管使用上面哪种试, 都需要在两个 NameNode 上分别格式化, 第一个 NameNode 可以使用之前的 `hdfs namenode -format` 命令进行格式化, 格式化后就会产生一个 **ClusterId**, 剩下的 NameNode 需要指定 **ClusterId** 进行格式化: `hdfs namenode -format -clusterId`.
